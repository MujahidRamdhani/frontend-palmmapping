...
const TambahPemetaan: React.FC = () => {
    ...
    const [hutanColors, setHutanColors] = useState<{ [key: string]: string }>(
        {},
    );
    const hutanColor = '#02ad05'; // Warna hijau muda
    ...
    useEffect(() => {
        setHutanColors((prevColors) => {
            const newColors = { ...prevColors };
            lapisanPeta.forEach((layer) => {
                if (
                    layer.id &&
                    layer.kawasan.startsWith('hutan') &&
                    !newColors[layer.id]
                ) {
                    newColors[layer.id] = hutanColor;
                }
            });
            return newColors;
        });
    ...
    useEffect(() => {
        ...
        if (newMaps.length > 0) {
            const filteredLayers = newMaps.filter(
                (layer) => layer.kawasan !== 'didalam kawasan2123',
            );
    ...
    const _onCreate = (e: any) => {
        ...
        if (layerType === 'polygon') {
            ...
            if (kawasan.startsWith('hutan')) {
                setHutanColors((prev) => ({
                    ...prev,
                    [_leaflet_id]: hutanColor,
                }));
            }
    ...
    const _onEdited = (e: any) => {
        ...
        const updatedLayers = Object.values(_layers).map((layer: any) => {
            ...
            // Pertahankan warna hutan
            const finalColor = kawasan.startsWith('hutan')
                ? hutanColors[_leaflet_id] || hutanColor
                : color;

            return { id: _leaflet_id, latlngs, color: finalColor, kawasan };
        });
        ...
        setLapisanPeta((prevLayers) => {
            ...
            return [...hutanLayers, ...updatedLapisanPeta];
        });
    };
                        ...
                        >
                                    ...
                                    <EditControl
                                        ...
                                        onEdited={(e) => {
                                            ...
                                            layers.eachLayer((layer) => {
                                                // Check if the layer represents a forest ('hutan')
                                                if (layer.options.isHutan) {
                                                    // Reapply the forest color
                                                    layer.setStyle({
                                                        color: 'green',
                                                    });
                                                }
                            ...
                            {lapisanPeta.map((layer, index) => (
                                <Polygon
                                    ...
                                    color={hutanColors[layer.id] || layer.color}
                                />
                            ))}
                        </MapContainer>
                    </div>
