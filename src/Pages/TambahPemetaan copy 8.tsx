...
const TambahPemetaan: React.FC = () => {
    ...
    const _onEdited = (e: any) => {
        ...
        setLapisanPeta((prevLayers) => {
            // Find the initial 'hutan' layer and keep it unchanged
            const hutanLayer = prevLayers.find(
                (layer) => layer.kawasan === 'hutan',
            );

            // Filter out layers that are being updated
            const nonUpdatedLayers = prevLayers.filter(
                (layer) =>
                    !updatedLayers.some(
            ...
            // Combine the unchanged 'hutan' layer (if it exists) with non-updated and updated layers
            const updatedLapisanPeta = hutanLayer
                ? [hutanLayer, ...nonUpdatedLayers, ...updatedLayers]
                : [...nonUpdatedLayers, ...updatedLayers];

            // Ensure the 'hutan' layer stays unchanged
            return updatedLapisanPeta.map((layer) =>
                layer.kawasan === 'hutan'
                    ? { ...layer, color: '#02ad05' }
                    : layer,
            );
