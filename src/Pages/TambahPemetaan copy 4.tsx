...
const TambahPemetaan: React.FC = () => {
    ...
    const [lapisanPeta2, setLapisanPeta2] = useState<PolygonLayer[]>([]);
    const [koordinatAwal, setKoordinatAwal] = useState<LatLngLiteral[]>([]);
    const [lapisanPeta, setLapisanPeta] = useState<PolygonLayer[]>([]);

    // useEffect(() => {
    //     if (dataPemetaanHutan.length > 0) {
    //         const formattedCoordinates = dataPemetaanHutan.map(d => {
    ...
    //         const newLapisanPeta = [
    //             { id: 1, latlngs: formattedCoordinates, color: 'green', kawasan: 'hutan' },
    //         ]

    //         setLapisanPeta(newLapisanPeta);
        ...
        const fetchDataPemetaanKebun = async () => {
            try {
                ...
                if (data.length > 0) {
                    ...
                    console.log('newLapisanPeta:', newLapisanPeta);
                    setLapisanPeta(newLapisanPeta);
                }
            } catch (error) {
    ...
    useEffect(() => {
        ...
        if (filteredLayers.length > 0) {
            ...
            if (
                ...
                filteredLayers.some(
                    (layer) => layer.kawasan === 'diluar kawasan',
                )
            ) {
            ...
            if (latlngs.length > 0) {
                latlngs.forEach((layer: any) => {
                    ...
                    layer.latlngs.forEach((innerLayer: any) => {
                        ...
                        dataLng.push(innerLayer.lng);
                    });
                });
            }

            console.log('Data Lat:', dataLat);
            console.log('Data Lng:', dataLng);
    ...
    const titikDalamPoligon = (
        ...
        console.log(`Point (${x}, ${y}) within polygon: ${diDalam}`);
        return diDalam;
    };
        ...
        const gamma =
            ((garis1[0].lat - garis1[1].lat) * (garis2[1].lng - garis1[0].lng) +
                (garis1[1].lng - garis1[0].lng) *
                    (garis2[1].lat - garis1[0].lat)) /
            det;
        const result = 0 < lambda && lambda < 1 && 0 < gamma && gamma < 1;
        console.log(
            `Line (${garis1[0].lat}, ${garis1[0].lng}) - (${garis1[1].lat}, ${garis1[1].lng}) intersects with (${garis2[0].lat}, ${garis2[0].lng}) - (${garis2[1].lat}, ${garis2[1].lng}): ${result}`,
        );
        return result;
    };
