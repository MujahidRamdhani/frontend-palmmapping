...
const Peta: React.FC = () => {
    ...
    const [dataPemetaan, setDataPemetaan] = useState<Cooperative[]>([]);
    ...
    const filterDataPenerbitan = (data: Cooperative[]) => {
        return data.filter(
            (item: Cooperative) => item.statusPenerbitanSTDB === 'Diterbitkan',
        );
    };

    const filteredStdbs = selectedCategory
        ? stdbs.filter((item) => item.statusKawasan === selectedCategory)
        : stdbs;
