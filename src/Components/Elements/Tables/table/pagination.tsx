...
const schema = z
    .object({
        ...
        statusKonfirmasiKoperasi: z.string(),
        uuid: z.string(),
        niKoperasi: z.string(),
        namaKoperasi: z.string(),
    })
    .refine(
        (data) => {
            // Check if statusKonfirmasiKoperasi indicates rejection
            if (data.statusKonfirmasiKoperasi === 'ditolak') {
...
export default function PaginationTablePage() {
    ...
    const onSubmit: SubmitHandler<FormFields> = async (data) => {
        try {
            ...
            const response = await axios.patch(
                ...
                {
                    niKoperasi: data.niKoperasi,
                    namaKoperasi: data.namaKoperasi,
                    statusKonfirmasiKoperasi: data.statusKonfirmasiKoperasi,
    ...
    if (user !== null) {
        ({ nik, nama } = user);
        setValue('niKoperasi', nik);
        setValue('namaKoperasi', nama);
    }
    ...
    const columns: ColumnDef<Cooperative>[] = [
        ...
        {
            accessorKey: 'statusKonfirmasiKoperasi',
    ...
    const filterData = (data: Cooperative[]) => {
        return data.filter(
            (item: Cooperative) =>
                item.statusKonfirmasiKoperasi === 'diterima' ||
                item.statusKonfirmasiKoperasi === 'ditolak',
        );
    };
    ...
    return (
        <main>
            ...
            {stdbs
                .filter((stdb) => stdb.uuid === uuid) // Filters cooperatives with statusKonfirmasiKoperasiKoperasi as "false" (string)
                .map((stdb, index) => (
                                        ...
                                        <div className="flex items-end justify-center min-h-full p-4 text-center sm:items-center sm:p-0">
                                            <div className="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all lg:w-2/4 sm:w-4/6 xl:w-1/2 ">
                                                ...
                                                <footer className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse mx-1">
                                                    ...
                                                    <button
                                                        className="inline-flex w-full justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-[#F87171] text-base font-medium text-white hover:bg-[#b32626] sm:ml-3 sm:w-auto sm:text-sm"
                                                        onClick={() => {
