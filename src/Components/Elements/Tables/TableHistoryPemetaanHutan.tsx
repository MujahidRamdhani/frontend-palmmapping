...
import {
    GetAllDataPemetaanHutan,
    HistoryPemetaanHutan,
} from '../../../Services/pemetaanHutanServices';
...
export default function PaginationTablePage() {
    ...
    useEffect(() => {
        const fetchDataPemetaanKebun = async () => {
            try {
                if (idHutanStore) {
                    const data = await HistoryPemetaanHutan(idHutanStore);
                    setDataPemetaanHutan(data);
                    console.log('data hutan', data);
                }
            } catch (error) {
    ...
    const columns: ColumnDef<DataPemetaanHutan>[] = [
        ...
        {
            accessorKey: 'updateWaktuPemetaanHutan',
            header: 'Waktu Update Pemetaan Hutan',
            cell: (props) => (
                <FormatTanggalJamLabel tanggal={props.getValue()} />
            ),
        },
                ...
                return (
                    <div className="">
                        <button
                            ...
                            onClick={() => {
                                setShowModal(true);
                                setIdHutan(cell.row.original.IdTransaksiBlockchain);
                            }}
                        >
                            Lihat
                        </button>
    ...
    return (
        <main>
            <div className="text-title-xxs font-semibold text-black dark:text-white mt-6">
                Riwayat Perubahan Pemetaan Hutan Dengan Id Hutan :{' '}
                {idHutanStore}
            </div>
            <PaginatedTable
                columns={columns}
            ...
            {dataPemetaanHutan
                .filter(
                    (dataPemetaanHutan) =>
                        dataPemetaanHutan.IdTransaksiBlockchain === idHutan,
                )
